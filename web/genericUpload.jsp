...
<c:choose>
	...
	<c:otherwise>
		<c:if test="${!empty contextValue}">
			<c:set var="contextDisplay" value="&nbsp;&nbsp;(${contextValue})"/>
		</c:if>
			...
			<tr>
				...
				<td valign="top" class="bodyText">
					...
					<c:if test='${isAdmin eq "1" and !empty contextValue}'>
						<c:set var="contextInfo" value="You have switched to the context <strong>${contextValue}</strong>." />
					</c:if>

					<table width="100%" cellspacing="0" cellpadding="0">
						<tr><td class="uploadHdr"><strong>File Upload Utility ${uploadKeyDisplay}</strong></td></tr>
						<tr><td class="uploadUserInfo">Hello, <strong>${winIsLoggedIn.name}</strong>. ${contextInfo}</td></tr>
					</table>
					<br/>
					...
					<c:choose>
						<c:when test="${!empty validationOK and !validationOK}">
							<span class="bodyText">We encountered errors in your file - your file was NOT uploaded.  After viewing the errors below,
							fix them in your original source file then upload it again to retry the validation.</span>
							<p/>
							<a class="validationLink" href="?appl=${appl}&config=${config}&key=${key}">Back to ${uploadName}</a>
							<p/>
							
							<span class="validationContentHdr">File Content</span><br/>
							<c:set var="cnt" value="0"/>
							<table cellspacing="2" cellpadding="2" class="validationTbl">
							
							<c:forEach items="${lineValidations}" var="line">
								<c:if test="${cnt == 0}">
									<tr>
										<c:forEach items="${line.content}" var="content">
											<c:set var="col" value="${content.value}" />
											<c:if test="${cnt == 0}">
												<th class="validationTblHdr">${col.column}</th>
											</c:if>									
										</c:forEach>
										<th class="validationTblHdr">Messages</th>
									</tr>
								</c:if>									

								<c:set var="rowSuffix" value=""/>
								<c:if test="${line.valid ne '0'}">
									<c:set var="rowSuffix" value="Err"/>
								</c:if>
								<tr class="validationRow${rowSuffix}">
									<c:forEach items="${line.content}" var="content">
										<c:set var="col" value="${content.value}" />
										<c:set var="classSuffix" value=""/>
										<c:if test="${col.valid ne '0'}">
											<c:set var="classSuffix" value="Err"/>
										</c:if>
										<td class="validationCell${classSuffix}">${col.data}</td>
									</c:forEach>
									<td class="validationCellMsg">
										<c:forEach items="${line.messages}" var="msg">
											<li>${msg}</li>
										</c:forEach>
									</td>
								</tr>								
							...
							<span class="bodyText"><strong>Validation Table Legend</strong></span>
							<table cellpadding="3">
								<tr class="validationRow">
									<td class="validationCell" style="border-collapse: collapse; border-top: 1px solid #ddd;">Value passed "column" validation</td>
								</tr>
								<tr class="validationRow">
									<td class="validationCellErr">Value failed "column" validation</td>
								</tr>
								<tr class="validationRowErr">
									<td class="validationCell">Line failed "row" validation</td>
								</tr>
								<tr class="validationRowErr">
									<td class="validationCellMsg"><li>Explanatory error message</td>
								</tr>
							</table>
									...
									<tr>
										<td width="50%" valign="top">
											...
											<c:forEach items="${directories}" var="d">
												...
												<c:if test="${dir.valid}">
													<span class="fileListHdr${classSuffix}">${dir.description}${contextDisplay}</span>
										...
										<td valign="top">
												...
												<% try { %>
													<c:forEach items="${fileUploads}" var="fU">
														...
														<form method="post" name="frm_${fileUpload.name}" enctype="multipart/form-data">
															...
															<span class="fileListHdr">${fileUpload.display}${contextDisplay}</span>
															<table with="100%" cellspacing="2" cellpadding="2" class="listTbl">
											...
											<c:if test='${hasActions eq "1" and keyMap.dirPathsValid}'>
												<span class="fileListHdr">Perform Actions${contextDisplay}</span>
											...
											<c:if test="${keyMap.pathValid and keyMap.dirPathsValid and (empty keyMap.runInfoValid or (!empty keyMap.runInfoValid and keyMap.runInfoValid))}">
												<span class="fileListHdr">Information Logs${contextDisplay}</span>
